...
window.addEventListener("message", (e) => {
  const t = e.data;
  t && "AH_PAGE" === t.src && "AH_READY" === t.type && (AH_BRIDGE_READY = !0);
});
...
const AH_STORE_KEYS = {
    ...
    stop: "ahStop",
    exp: "ahExpSec",
    migrated: "ahMigratedV2",
  },
  AH_TMP_HEAVY = ["ahStatsDaily", "ahFTResume"],
  AH_STORE = {
    ready: !1,
    readyPromise: null,
...
async function tryRescueFromLastPayload() {
  const e = (await st.get(["ahLastPayload"])).ahLastPayload;
...
(sendTokenToSW(),
  ...
  `),
  chrome.storage.local.get("ahAutoBackup", (e) => {
    e.ahAutoBackup &&
...
async function buildModal() {
  ...
  document.body.append(elt("div", { id: "ah-overlay" }, n));
  expandModal();
}
...
function expandModal() {
  ...
  e.insertBefore(t, $("#ah-form"));
  e.append(elt("div", { id: "ah-body" }));
  selectTab(0);
  updateHeaderExp();
}
function cleanupAuthUI() {
  ($("#ah-form")?.remove(), $("#ah-res")?.remove());
}
function selectTab(e) {
  ...
  // Вкладки Main (0), Invites (1), Letters (2) — показываем заглушку
  if (0 === e || 1 === e || 2 === e) {
    ...
    const overlay = elt("div", {
      style: [
        ...
        "background:rgba(245,246,250,0.82);backdrop-filter:blur(2px);",
        "z-index:10;gap:18px;",
      ].join("")
    });

    const lockIcon = elt("div", { style: "font-size:52px;line-height:1;" }, "🔒");

    const msg = elt("div", {
      style: [
        "font-size:17px;font-weight:700;color:#636e72;text-align:center;",
        "max-width:420px;line-height:1.5;",
      ].join("")
    }, "Авторассылка доступна в платном боте.");

    const subMsg = elt("div", {
      style: "font-size:14px;color:#b2bec3;text-align:center;max-width:380px;line-height:1.5;"
    }, "Следи за новостями в Расходнике.");
    ...
    tgBtn.onmouseenter = () => { tgBtn.style.transform = "translateY(-2px)"; tgBtn.style.boxShadow = "0 10px 28px rgba(0,136,204,0.45)"; };
    tgBtn.onmouseleave = () => { tgBtn.style.transform = ""; tgBtn.style.boxShadow = "0 6px 20px rgba(0,136,204,0.35)"; };

    const tgIcon = elt("span", { style: "font-size:20px;" }, "✈️");
    const tgText = elt("span", {}, "TG: @brachka_rass");
    tgBtn.append(tgIcon, tgText);

    overlay.append(lockIcon, msg, subMsg, tgBtn);
...
function startLettersTimer(pid, items) {
  ...
  const tick = () => {
    chrome.storage.local.get(["ahRotationState", "ahLastStatsTime"], (res) => {
      ...
      const s = loadSet();
      const isServerDead = Date.now() - (res.ahLastStatsTime || 0) > 6000;

      if (!s.running || isServerDead) return; // Если стоп или сервер упал -> выходим

      // 3. Рисуем таймер
      const rotation = res.ahRotationState || {};
...
function renderTools(e) {
  ...
  const fileInput = elt("input", {
    ...
    onchange: ev => {
      ...
      reader.onload = event => {
        ...
        settings.watermarkCustomImage = event.target.result;
        saveSet(settings);
        uploadBtn.textContent = "✓ Загружено";
        setTimeout(() => { uploadBtn.textContent = "Выбрать"; }, 2000);
      };
      reader.readAsDataURL(file);
    }
  });

  const uploadBtn = elt("button", {
    style: [
      "padding:6px 14px;background:#fff;",
      "color:var(--sa,#6c5ce7);border:1px solid var(--sa,#6c5ce7);",
  ...
  const resetWmBtn = elt("button", {
    style: [
      "padding:6px 14px;background:#fff;color:#b2bec3;",
      "border:1px solid #e8e8e8;border-radius:8px;",
      "font-size:12px;cursor:pointer;",
      "font-family:'Inter',system-ui,sans-serif;transition:.15s;",
    ].join(""),
    onclick: () => {
      if (!confirm("Вернуть стандартный логотип?")) return;
      const settings = loadSet();
      delete settings.watermarkCustomImage;
      saveSet(settings);
    },
  }, "Сброс");

  // Водяной знак — через loadSet/saveSet (как makeSwitch)
  const mkLocalToggle = (label, key, defaultVal) =>
    mkToggleRow(label,
      cb => { const s = loadSet(); cb(s[key] !== undefined ? s[key] : defaultVal); },
      v  => { const s2 = loadSet(); s2[key] = v; saveSet(s2); }
    );

  // Прозрачность водяного знака
  const settingsNow = loadSet();
  const curOpacity = settingsNow.watermarkOpacity !== undefined ? settingsNow.watermarkOpacity : 0.1;
  const opacityVal = elt("span", { style: "font-size:12px;font-weight:700;color:var(--sa,#6c5ce7);min-width:32px;text-align:right;font-family:'Inter',system-ui,sans-serif;" }, Math.round(curOpacity * 100) + "%");
  const opacitySlider = elt("input", {
    type: "range", min: "0", max: "100", value: Math.round(curOpacity * 100),
    style: "flex:1;margin:0 12px;cursor:pointer;accent-color:var(--sa,#6c5ce7);",
    oninput: ev => {
      const v = parseInt(ev.target.value) / 100;
      opacityVal.textContent = Math.round(v * 100) + "%";
      const s = loadSet(); s.watermarkOpacity = v; saveSet(s);
    },
  });
  const opacityRow = elt("div", { style: "display:flex;align-items:center;padding:10px 0;border-bottom:1px solid #f7f7f7;font-family:'Inter',system-ui,sans-serif;" },
    elt("span", { style: "font-size:13px;color:#3a3a3a;font-weight:450;min-width:120px;" }, "Прозрачность"),
    opacitySlider, opacityVal,
  );

  const wmImageRow = elt("div", { style: "display:flex;align-items:center;justify-content:space-between;padding:10px 0;font-family:'Inter',system-ui,sans-serif;" },
    elt("span", { style: "font-size:13px;color:#3a3a3a;font-weight:450;" }, "Изображение"),
    elt("div", { style: "display:flex;gap:6px;" }, uploadBtn, resetWmBtn, fileInput),
  );

  // Транслит — тумблер убран, кнопка Aa всегда видна

  container.append(mkCard("Интерфейс",
    mkLocalToggle("Водяной знак в чате", "watermarkEnabled", true),
    opacityRow,
    wmImageRow,
  ));
  ...
  // Создаем карточку для скачивания
  const downloadCard = elt("div", {
    className: "ah-card",
    style: "min-height: 200px; display:flex; flex-direction:column",
  });
  downloadCard.append(
    elt("div", { className: "ah-card-title" }, "📥 Скачать историю / Инфо"),
  );
  ...
  if (p && p[1]) {
    ...
    searchRow.append(loadViewBtn, searchInput, searchBtn);
    ...
    function renderChatView(filter = "") {
      ...
      const filtered = q
        ? allMessages.filter(m => (m.text || "").toLowerCase().includes(q) || (m.author || "").toLowerCase().includes(q))
        : allMessages;
      ...
      filtered.forEach(m => {
        ...
        const bubble = elt("div", {
          style: `max-width:80%;padding:6px 10px;border-radius:${borderRadius};background:${bubbleBg};border:1px solid ${bubbleBorder};word-break:break-word;`,
        });
    ...
    searchInput.addEventListener("keydown", (e) => { if (e.key === "Enter") renderChatView(searchInput.value); });
...
function renderStats(e) {
  ...
  const update = () => {
    chrome.storage.local.get("ahDailyStats", (res) => {
      const s = res.ahDailyStats || { chat: 0, letters: 0, errors: 0 };
...
function resetAuthUI() {
  // Функция переделана, чтобы не сбрасывать авторизацию локально
  console.log("Локальный обход: блокировка сброса интерфейса.");
  /* Оригинальный код удаления данных закомментирован, 
    чтобы бот не вылетал из аккаунта.
  */
}
function openStarMenu(e) {
  ...
  const a = elt(
    ...
    (() => {
      ...
      return (
        chrome.storage.local.get("ahAutoBackup", (t) => {
          e.checked = !!t.ahAutoBackup;
        }),
        (e.onchange = (e) => {
          (chrome.storage.local.set({ ahAutoBackup: e.target.checked }),
            chrome.runtime.sendMessage({
...
function openPort() {
  if (!port) {
    try {
      port = chrome.runtime.connect({ name: "KEEP_ALIVE_AH" });
    } catch (e) {
                  ...
                  const o = "ah_" + Math.random().toString(36).slice(2),
                    ...
                    i = new Promise((e, t) => {
                      ...
                      var i = setTimeout(() => {
                        ...
                        try {
                          window.postMessage(
                            { src: "AH_SW", type: "AH_FETCH_CANCEL", id: o },
                            location.origin,
                          );
                        } catch {}
                        t(new Error("timeout"));
...
// ═══════════════════════════════════════════════════════════
(function initEarningsPanelHooks() {

  // Применяем цвет темы к бейджу начисления
  function watchBadge() {
    const themeColor = () =>
      getComputedStyle(document.documentElement).getPropertyValue('--sa').trim() || '#FF6B35';
    ...
    function findAndPaint() {
      document.querySelectorAll('.ah-earnings-portal *').forEach(el => {
        try {
          if (getComputedStyle(el).backgroundColor === 'rgb(255, 107, 53)') paintBadge(el);
        } catch {}
      });
    }

    // Observer: ловим и добавление узлов, и изменение class/style
    const badgeObs = new MutationObserver((mutations) => {
      let needPaint = false;
      for (const m of mutations) {
        ...
        if (m.addedNodes.length) {
          for (const node of m.addedNodes) {
            ...
            try {
              if (getComputedStyle(node).backgroundColor === 'rgb(255, 107, 53)') {
                paintBadge(node);
              }
        ...
        // Изменение class или style — Chakra перезаписала стиль
        if (m.type === 'attributes') {
          needPaint = true;
        }
      }
      if (needPaint) findAndPaint();
    });

    badgeObs.observe(document.body, {
      childList: true,
      subtree: true,
      attributes: true,
    ...
    // Дополнительный интервал как страховка — каждые 2 секунды
    setInterval(findAndPaint, 2000);
  }

  // Добавляем свою кнопку-шестерёнку поверх оригинальной
  function injectSettingsBtn() {
    const portal = document.querySelector('.ah-earnings-portal');
    if (!portal) return;
    if (portal.querySelector('.sn-earn-settings-btn')) return;

    // Скрываем оригинальную шестерёнку через атрибут
    const origBtn = portal.querySelector('button[aria-label="Settings"]');
    if (origBtn) origBtn.style.display = 'none';

    const btn = document.createElement('button');
    btn.className = 'sn-earn-settings-btn';
    btn.title = 'Настройки в Tools';
    btn.innerHTML = `<svg viewBox="0 0 14 14" width="12" height="12" fill="currentColor">
      <path d="M14,7.77 L14,6.17 L12.06,5.53 L11.61,4.44 L12.49,2.6 L11.36,1.47 L9.55,2.38 L8.46,1.93 L7.77,0.01 L6.17,0.01 L5.54,1.95 L4.43,2.4 L2.59,1.52 L1.46,2.65 L2.37,4.46 L1.92,5.55 L0,6.23 L0,7.82 L1.94,8.46 L2.39,9.55 L1.51,11.39 L2.64,12.52 L4.45,11.61 L5.54,12.06 L6.23,13.98 L7.82,13.98 L8.45,12.04 L9.56,11.59 L11.4,12.47 L12.53,11.34 L11.61,9.53 L12.08,8.44 L14,7.75 Z M7,10 C5.34,10 4,8.66 4,7 C4,5.34 5.34,4 7,4 C8.66,4 10,5.34 10,7 C10,8.66 8.66,10 7,10 Z"/>
    </svg>`;
    btn.style.cssText = `
      background:none; border:none; cursor:pointer; padding:2px 4px;
      opacity:0.4; transition:opacity .15s; color:inherit;
    ...
    // Вставляем на место оригинальной шестерёнки
    if (origBtn) {
      origBtn.parentElement.insertBefore(btn, origBtn);
    }
  }

  function scrollToEarningsCard() {
    setTimeout(() => {
      const cards = document.querySelectorAll('.ah-single-wrapper > div');
      for (const card of cards) {
        if (card.textContent?.includes('Уведомления')) {
  ...
  let hookTimer = null;
  const earningsObs = new MutationObserver(() => {
  ...
  earningsObs.observe(document.body, { childList: true, subtree: true });
  ...
  function injectDialogsCounter() {
    ...
    for (const el of portal.querySelectorAll('*')) {
      ...
      if (/^\d{4}-\d{2}-\d{2}\s*\(UTC\)$/.test(txt) && el.children.length === 0) {
        dateEl = el;
        break;
      }
    }
  ...
  function updateDialogsCounter() {
    chrome.storage.local.get(['ahDialogsFound', 'ahLastStatsTime', 'ahRunning'], r => {
      const n       = r.ahDialogsFound  || 0;
      const lastTs  = r.ahLastStatsTime || 0;
  ...
  // Обновляем счётчик при изменении storage (новый скан или смена статуса)
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'local' && ('ahDialogsFound' in changes || 'ahRunning' in changes || 'ahLastStatsTime' in changes)) {
      updateDialogsCounter();
    }
  });
  ...
  injectDialogsCounter();
  watchBadge();
  // Обновляем статус каждые 10 сек (чтобы кружок стал серым если сервер упал)
  setInterval(updateDialogsCounter, 10000);
})();

// ═══════════════════════════════════════════════════════════
// АВТООБНОВЛЕНИЕ ЧИСЛОВЫХ ЛИМИТОВ В СПИСКЕ ЧАТОВ
// Проблема: сайт запрашивает chatListByUserID только при загрузке.
// Решение: периодически запрашиваем сами и обновляем localStorage.
// ═══════════════════════════════════════════════════════════
