...
!(function () {
  ...
  try {
    window.postMessage({ src: "AH_PAGE", type: "AH_READY" }, location.origin);
  } catch {}
  window.addEventListener("message", async (i) => {
    const s = i.data;
    if (!s || "AH_SW" !== s.src) return;
    if ("AH_PING" === s.type) {
      try {
        window.postMessage(
          { src: "AH_PAGE", type: "AH_READY" },
          location.origin,
        );
      } catch {}
      return;
    }
    if ("AH_FETCH_CANCEL" === s.type && s.id) {
